                    ...
                    <ul class="list-disc space-y-2">
                        @foreach ($user->applications as $application)
                            @if ($application->accepted === 0)
                                <li
                                    class="border rounded-lg p-4 shadow bg-cream flex flex-col sm:flex-row justify-between items-start sm:items-center py-2">
                                    ...
                                    <div class="flex items-center space-x-4 mt-2 sm:mt-0">
                                        <span class="text-gray-800">
                                            ...
                                            @php
                                                $applications = $application->vacature->applications->where(
                                                    'accepted',
                                                    0,
                                                );
                                        ...
                                        <div class="flex flex-col md:flex-row gap-3">
                                            <a href="{{ route('vacatures.show', $application->vacature_id) }}"
                                                class="no-underline inline-flex items-center px-4 py-2 md:px-6 md:py-3 md:text-xs bg-violet-light border border-transparent rounded-md font-semibold text-[0.6rem] text-cream uppercase tracking-widest hover:bg-violet-dark focus:bg-violet-dark active:bg-violet-dark focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">Zie
                                                vacature</a>
